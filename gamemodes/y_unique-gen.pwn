...
Write@X__(a, base)
{
	...
	format(name, sizeof (name), "DANGEROUS_SERVER_ROOT/pawno/include/YSI_Coding/y_unique/y_unique_%04d_%04d.inc", base, base + 64 - 1);
	...
	if (f)
	{
		...
		format(name, sizeof (name), "static stock const Y_UNIQUE_%04d_%04d_CALLED = UNIQUE_SYMBOL;\n", base, base + 64 - 1), fwrite(f, name);
		format(name, sizeof (name), "\n"), fwrite(f, name);
		format(name, sizeof (name), "#if defined _inc_y_unique_%04d_%04d\n", base, base + 64 - 1), fwrite(f, name);
		format(name, sizeof (name), "	#undef _inc_y_unique_%04d_%04d\n", base, base + 64 - 1), fwrite(f, name);
		format(name, sizeof (name), "#endif\n"), fwrite(f, name);
		format(name, sizeof (name), "\n"), fwrite(f, name);
		for (new i = 0; i != 8; ++i)
		{
			...
			for (new j = 0; j != 8; ++j)
			{
				if (j == 7)
					...
					format(name, sizeof (name), "	#%sif UNIQUE_SYMBOL == (%d)\n", j ? "else" : "", base + i * 8 + j - 1), fwrite(f, name);
				format(name, sizeof (name), "		#undef UNIQUE_SYMBOL\n"), fwrite(f, name);
				format(name, sizeof (name), "		#define UNIQUE_SYMBOL (%d)\n", base + i * 8 + j), fwrite(f, name);
				format(name, sizeof (name), "		#define UNIQUE_FUNCTION<%%0...%%1> %%0%c%c%%1\n", a, ch[i * 8 + j]), fwrite(f, name);
	...
	if (f)
	{
		...
		for (new i = 0; i != 8; ++i)
		{
			if (i == 7)
				...
				format(name, sizeof (name), "	#%sif UNIQUE_SYMBOL < (%d)\n", i ? "else" : "", i * 64 * 8 + 64 * 8 - 1), fwrite(f, name);
			for (new j = 0; j != 8; ++j)
			{
				if (j == 7)
					format(name, sizeof (name), "		#else\n"), fwrite(f, name);
				else
					format(name, sizeof (name), "		#%sif UNIQUE_SYMBOL < (%d)\n", j ? "else" : "", i * 64 * 8 + j * 64 + 64 - 1), fwrite(f, name);
				format(name, sizeof (name), "			static stock const UNIQUE_SYMBOL_LESS_THAN_%04d = UNIQUE_SYMBOL;\n", i * 64 * 8 + j * 64 + 64 - 1), fwrite(f, name);
				format(name, sizeof (name), "			#include \"y_unique_%04d_%04d\"\n", i * 64 * 8 + j * 64, i * 64 * 8 + j * 64 + 64 - 1), fwrite(f, name);
				format(name, sizeof (name), "			#endinput\n"), fwrite(f, name);
			}
			format(name, sizeof (name), "		#endif\n"), fwrite(f, name);
		}
